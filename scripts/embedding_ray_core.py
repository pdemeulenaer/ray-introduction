...
# EMBEDDING AS AN ACTOR USING RAY CORE

@ray.remote(num_cpus=4, num_gpus=1)
class EmbeddingClass2:
    def __init__(self): 
        # If doing CPU inference, set `device="cpu"` instead.
        self.img_model = SentenceTransformer('clip-ViT-B-32', device='cuda')
    ...
    def compute(self, images):
        ...
        if isinstance(images, list):
            if isinstance(images[0], Image.Image):
                vectors = self.img_model.encode(images)
            else:
                images_pil = [ Image.fromarray(bytes_to_array(image)) for image in images ]
                vectors = self.img_model.encode(images_pil)
...
embeddingsCalc = EmbeddingClass2.remote()

# Put the images in the shared object store
ref_images = ray.put(images)

# Call the actor
start = time.time()
x_vectors_ref = embeddingsCalc.compute.remote(ref_images)
vectors = ray.get([x_vectors_ref])
print(vectors[0][0])
